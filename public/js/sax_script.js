...
var signature = signaturePad.toDataURL(); //as PNG
    ...
    form_data.append("sing",  signature); 
      ...
      $.when(set_sol_to_delivered()).done(function( RES ){ 
        ...
        if(RES.trim() == 'ok'){
           ...
           MSG_ERROR('SE PRODUJO UN ERROR AL GUARDAR LOS CAMBIOS, CONSULTE CON EL ADMINSITRADOR DEL SISTEMA - '+res, 0);
...
//VER FIRMA Y FOTO
function view_modal(id_sol,id_item){


            $.ajax({
                  type: "GET",
                  url: link,
                  data:  { url : 'bridge_query/view_singed_info/'+id_sol+'/'+id_item } , 
                  success: function(dat){
                  
                  console.log(dat);
                  
                  var info = JSON.parse(dat);
                  document.getElementById("RcvName").value = info.USER_RECV;

                  $('#ViewSing').attr('src', URL+'img/Signatures/'+info.NO_SOL+'-'+id_item+'.png');
                  $('#ViewImage').attr('src', URL+'img/Del_item_img/'+info.NO_SOL+'-'+id_item+'.jpg');
